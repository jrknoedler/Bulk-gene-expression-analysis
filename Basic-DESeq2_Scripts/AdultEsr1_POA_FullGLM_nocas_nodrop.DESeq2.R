...
cond2 <- "Primed"
cond3 <- "Unprimed"
output <- "DESeq2Scripts/Esr1Adult_POA_GLM_3way_dropY_dropoutlier"
...
samples <- read.table("Kallistosheets/AdultEsr1Kallisto_POAFULL_nooutlier.tsv", header=TRUE)
samples
...
sampleTable <- data.frame(condition = factor(c(rep(cond1, 3), rep(cond2, 2), rep(cond3, 3))))
