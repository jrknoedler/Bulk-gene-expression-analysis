...
cond2 <- "PrimedPOA"
cond3 <- "UnprimedPOA"
output <- "DESeq2Scripts/Esr1Adult_POA_GLM"
...
samples <- read.table("Kallistosheets_cp/AdultEsr1Kallisto_POAFULL.tsv", header=TRUE)
samples
...
sampleTable <- data.frame(condition = factor(c(rep(cond1, 3), rep(cond2, 3), rep(cond3, 3))))
...
data <- mcols(dds)
write.csv(data, file=paste0(output,"localdisp.csv"))
ddsparam <- estimateDispersions(dds,fitType="parametric")
data2 <- mcols(ddsparam)
write.csv(data2,file=paste0(output,"paramdisp.csv"))
dds <- DESeq(dds)	
mcols(dds)
dispGeneEst <-na.omit(dds$dispGeneEst)
dispGeneEst <- log(dispGeneEst)
dispGeneEst																												
dispGeneEst
# run DESeq2
resultsNames(dds)
...
dispdiff <-  median(abs(log(dds$dispGeneEst)-log(dds$dispFit)))
dispdiff
