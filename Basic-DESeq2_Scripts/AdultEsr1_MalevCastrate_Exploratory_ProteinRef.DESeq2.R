...
cond7 <- "IntactVMH"
cond8 <- "CastrateVMH"
output <- "DESeq2Scripts/Esr1Adult_MalevCastrate_exploratory_region"
...
samples <- read.table("Kallistosheets_cp/MalevCastrate_all.tsv", header=TRUE)
samples
...
sampleTable <- data.frame(condition = factor(c(rep(cond1, 3), rep(cond2, 3), rep(cond3, 3), rep(cond4, 3), rep(cond5, 3), rep(cond6, 3), rep(cond7, 3), rep(cond8, 3))), batch= factor(c(rep("1"), rep("1"),rep("1"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("1"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("1"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("1"),rep("1"),rep("1"),rep("2"))), region=factor(c(rep("BNST",6), rep("MeA",6), rep("POA",6), rep("VMH", 6))))
rownames(sampleTable) <- colnames(txi.kallisto$counts)


# Run DESeq2 to get the result for differential-expression
dds=DESeqDataSetFromTximport(txi.kallisto, sampleTable, design =~batch+region+condition)
