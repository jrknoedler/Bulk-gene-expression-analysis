...
cond2 <- "Primed"
cond3 <- "Unprimed"
output <- "DESeq2Scripts/Esr1Adult_BNST_GLM_3way_dropY_droplow_dropmtbatchfemale"
...
samples <- read.table("Kallistosheets/AdultEsr1Kallisto_BNST_rean.tsv", header=TRUE)
samples
...
sampleTable <- data.frame(condition = factor(c(rep(cond1, 3), rep(cond2, 3), rep(cond3, 3))), batch = factor(c(rep("1"), rep("1"), rep("1"), rep("2"), rep("2"), rep("1"), rep("2"), rep("2"), rep("1"))))
