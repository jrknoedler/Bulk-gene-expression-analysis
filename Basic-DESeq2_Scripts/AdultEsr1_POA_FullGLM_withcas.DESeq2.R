...
cond2 <- "PrimedPOA"
cond3 <- "UnprimedPOA"
cond4 <- "CastrateMalePOA"
output <- "DESeq2Scripts/Esr1Adult_POA_GLM_4way_Yexclude_droplow_dropmt_heatmap_batch3"
...
samples <- read.table("Kallistosheets/AdultEsr1Kallisto_POAFULLcas.tsv", header=TRUE)
samples
...
sampleTable <- data.frame(condition = factor(c(rep(cond1, 3), rep(cond2, 3), rep(cond3, 3), rep(cond4, 2))), batch = factor(c(rep("1"), rep("1"), rep("2"), rep("1"), rep("1"), rep("2"), rep("3"), rep("3"), rep("2"), rep("1"), rep("1"))))
