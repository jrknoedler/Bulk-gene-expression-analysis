...
output <- "DESeq2Scripts/Esr1Adult_BNST_GLM_3way_redo_dropY_droplow_heatmaptest_dropmt_batchinvert"
...
samples <- read.table("Kallistosheets/AdultEsr1Kallisto_BNST_red.tsv", header=TRUE)
samples
files <- file.path(dir, samples$sample, "abundance.h5")
files
names(files) <- paste0(samples$name, 1:9)
...
sampleTable <- data.frame(condition = factor(c(rep(cond1, 3), rep(cond2, 3), rep(cond3, 3))), batch = factor(c(rep("1"), rep("1"), rep("1"), rep("1"), rep("1"), rep("1"), rep("2"), rep("2"), rep("1"))))
...
write.csv(cooks, file=paste0(output,"_cooksdistance.csv"))
...
#names(resPrimedvCastrate)[1] <- "Gene"
## Write resuls
#write.csv(resPrimedvCastratedata, file=paste0(output, "UvC-ExpDiff.csv"))
