...
cond3 <- "Unprimed"
cond4 <- "CastrateMale"
output <- "DESeq2Scripts/Esr1Adult_BNST_GLM_4way_redo_dropY_droplow_heatmaptest_dropmt_reanalyzed"
...
dir <- "/scratch/users/knoedler/BNST_Reanalyzed/"
samples <- read.table("Kallistosheets/AdultEsr1Kallisto_BNST_FULL.tsv", header=TRUE)
samples
files <- file.path(dir, samples$sample, "abundance.h5")
files
names(files) <- paste0(samples$name, 1:11)
...
sampleTable <- data.frame(condition = factor(c(rep(cond1, 3), rep(cond2, 3), rep(cond3, 3), rep(cond4, 2))))
...
plotPCA.DESeqTransform(rld, intgroup="condition",ntop=500)
