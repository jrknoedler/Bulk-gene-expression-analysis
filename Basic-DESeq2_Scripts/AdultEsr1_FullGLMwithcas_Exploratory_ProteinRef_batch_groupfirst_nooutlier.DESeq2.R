...
cond15 <- "CastratePOA"
cond16 <- "CastrateVMH"
output <- "DESeq2Scripts/Esr1Adult_FULL_GLM_withcast_original_groupfirst_nooutlier"
...
samples <- read.table("Kallistosheets_cp/AdultEsr1Kallisto_FULL_nocastoutlier.tsv", header=TRUE)
samples
files <- file.path(dir, samples$sample, "abundance.h5")
files
names(files) <- paste0(samples$name, 1:44)
tx2gene <- read.table("/home/groups/nirao/Bioinformatics3/Bioinformatics/vM21_Tx2Gene_Final.txt")
head(tx2gene)
txi.kallisto <- tximport(files, type="kallisto", tx2gene=tx2gene)

sampleTable <- data.frame(group=factor(c(rep("Primed", 3), rep("Unprimed", 3), rep("IntactMale",12), rep("Primed", 9), rep("Unprimed", 9), rep("CastrateMale", 8))), condition = factor(c(rep(cond1, 3), rep(cond2, 3), rep(cond3, 3), rep(cond4, 3), rep(cond5, 3), rep(cond6, 3), rep(cond7, 3), rep(cond8, 3), rep(cond9, 3), rep(cond10, 3), rep(cond11, 3), rep(cond12, 3), rep(cond13, 2), rep(cond14, 2), rep(cond15, 2), rep(cond16, 2))), batch= factor(c(rep("1"), rep("1"),rep("2"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("2"), rep("1"),rep("1"),rep("1"),rep("1"),rep("1"),rep("1"),rep("1"),rep("1"))), region= factor(c(rep("BNST", 9), rep("MeA",3), rep("POA",3), rep("VMH",3), rep("MeA",3), rep("POA",3), rep("VMH",3), rep("MeA",3), rep("POA",3), rep("VMH",3), rep("BNST", 2), rep("MeA",2), rep("POA",2), rep("VMH",2))), dissection.nested = factor(c(rep("1"), rep("2"), rep("3"), rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"), rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("1"), rep("2"),rep("1"), rep("2"), rep("1"), rep("2"))))
rownames(sampleTable) <- colnames(txi.kallisto$counts)

# Run DESeq2 to get the result for differential-expression
dds=DESeqDataSetFromTximport(txi.kallisto, sampleTable, design=~0+group:region+group:dissection.nested)
dds <- estimateSizeFactors(dds)
nc <- counts(dds, normalized=TRUE)
filter <- rowSums(nc >= 10) >= 14
dds <- dds[filter,]
