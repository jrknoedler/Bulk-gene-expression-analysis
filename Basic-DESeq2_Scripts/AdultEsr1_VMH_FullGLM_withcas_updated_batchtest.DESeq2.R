...
cond3 <- "Unprimed"
cond4 <- "CastrateMale"
output <- "DESeq2Scripts/Esr1Adult_VMH_GLM_4way_dropY_droplow_dropmt_updatedcastrate_batchtest"
...
samples <- read.table("Kallistosheets/AdultEsr1Kallisto_VMH_FULL_updatedcas_batchtest.tsv", header=TRUE)
samples
files <- file.path(dir, samples$sample, "abundance.h5")
files
names(files) <- paste0(samples$name, 1:14)
...
sampleTable <- data.frame(condition = factor(c(rep(cond1, 3), rep(cond2, 3), rep(cond3, 3), rep(cond4, 5))))
