...
cond7 <- "MalePOA"
cond8 <- "MaleVMH"
output <- "DESeq2Scripts/Esr1Adult_PrimedvMale_GLM_region_dissection_local"
...
samples <- read.table("Kallistosheets_cp/AdultEsr1Kallisto_MalevPrimed_GLM.txt", header=TRUE)
samples
files <- file.path(dir, samples$sample, "abundance.h5")
files
names(files) <- paste0(samples$name, 1:24)
tx2gene <- read.table("/home/groups/nirao/Bioinformatics3/Bioinformatics/vM21_Tx2Gene_Final.txt")
head(tx2gene)
txi.kallisto <- tximport(files, type="kallisto", tx2gene=tx2gene)

sampleTable <- data.frame(condition=factor(c(rep(cond1, 3), rep(cond2, 3), rep(cond3, 3), rep(cond4, 3), rep(cond5, 3), rep(cond6, 3), rep(cond7, 3), rep(cond8, 3))), group = factor(c(rep("Primed", 12), rep("IntactMale",12))), region = factor(c(rep("BNST", 3), rep("MeA", 3), rep("POA", 3), rep("VMH", 3), rep("BNST", 3), rep("MeA", 3), rep("POA", 3), rep("VMH", 3))), dissection=factor(c(rep("1"),rep("2"),rep("3"),rep("1"),rep("2"),rep("3"),rep("1"),rep("2"),rep("3"),rep("1"),rep("2"),rep("3"),rep("7"),rep("5"),rep("6"),rep("4"),rep("5"),rep("6"),rep("4"),rep("5"),rep("6"),rep("4"),rep("5"),rep("6"))), dissection.nested = factor(c(rep("1"), rep("2"), rep("3"), rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"))))
rownames(sampleTable) <- colnames(txi.kallisto$counts)


# Run DESeq2 to get the result for differential-expression
dds=DESeqDataSetFromTximport(txi.kallisto, sampleTable, design=~0+condition+region+group:dissection.nested)
...
resBNST <- results(dds, contrast=c(cond1, cond5))
...
resMeA <- results(dds, contrast=c(cond2,cond6))
...
resPOA <- results(dds, contrast=c(cond3,cond7))
...
resVMH <- results(dds, contrast=c(cond4,cond8))
