...
cond1 <- "P15_IP"
cond2 <- "P15_Input"
output <- "All_RNASeq_Analyses/DESeq2/Kallisto_P15_AllIPvInput_droplow"
...
samples <- read.table("NewKallistoSheets/AllP15Esr1Kallisto.txt", header=TRUE)
samples
files <- file.path(dir, samples$sample, "abundance.h5")
files
names(files) <- paste0(samples$name, 1:32)
tx2gene <- read.table("Bioinformatics3/Bioinformatics/vM17.tx2gene.FINAL.txt")
head(tx2gene)
txi.kallisto <- tximport(files, type="kallisto", tx2gene=tx2gene)

sampleTable <- data.frame(condition = factor(c(rep(cond1, 16), rep(cond2, 16))))
