...
output <- "DESeq2Scripts/All_Esr1adult_RegionComparison"
...
samples <- read.table("Kallistosheets/AllAdultEsr1_nocas.tsv", header=TRUE)
samples
...
sampleTable <- data.frame(region = factor(c(rep(cond1, 9), rep(cond2, 9), rep(cond3, 9), rep(cond4, 9))))
rownames(sampleTable) <- colnames(txi.kallisto$counts)


# Run DESeq2 to get the result for differential-expression
dds=DESeqDataSetFromTximport(txi.kallisto, sampleTable, design =~region)
...
#Save DE results
resPOAvBNST<- results(dds, contrast=c("region",cond1,cond2))
table(resPOAvBNST$padj<sigpadj)
## Order by adjusted p-value
resPOAvBNST <- resPOAvBNST[order(resPOAvBNST$padj), ]
## Merge with normalized count data
resPOAvBNSTdata <- merge(as.data.frame(resPOAvBNST), as.data.frame(counts(dds, normalized=TRUE)), by="row.names", sort=FALSE)
names(resPOAvBNST)[1] <- "Gene"
## Write results
write.csv(resPOAvBNSTdata, file=paste0(output, "_POAvBNST-ExpDiff.csv"))

resPOAvMeA <- results(dds, contrast=c("region",cond1,cond3))
table(resPOAvMeA$padj<sigpadj)
## Order by adjusted p-value
resPOAvMeA <- resPOAvMeA[order(resPOAvMeA$padj), ]
## Merge with normalized count data
resPOAvMeAdata <- merge(as.data.frame(resPOAvMeA), as.data.frame(counts(dds, normalized=TRUE)), by="row.names", sort=FALSE)
names(resPOAvMeA)[1] <- "Gene"
## Write results
write.csv(resPOAvMeAdata, file=paste0(output, "_POAvMeA-ExpDiff.csv"))

resPOAvVMH <- results(dds, contrast=c("region",cond1,cond4))
table(resPOAvVMH$padj<sigpadj)
## Order by adjusted p-value
resPOAvVMH <- resPOAvVMH[order(resPOAvVMH$padj), ]
## Merge with normalized count data
resPOAvVMHdata <- merge(as.data.frame(resPOAvVMH), as.data.frame(counts(dds, normalized=TRUE)), by="row.names", sort=FALSE)
names(resPOAvVMH)[1] <- "Gene"
## Write results
write.csv(resPOAvVMHdata, file=paste0(output, "_POAvVMH-ExpDiff.csv"))

# Principal Components Analysis
pdf(paste0(output, "all-PCA_top500.pdf"))
plotPCA(rld, intgroup="condition",ntop=500)
garbage <- dev.off() # Save to file
pdf(paste0(output, "all-PCA_top100.pdf"))
plotPCA(rld, intgroup="condition",ntop=100)
garbage <- dev.off() # Save to file
pdf(paste0(output, "all-PCA_top1000.pdf"))
...
sig <- rownames(respairwise[!is.na(respairwise$padj) & respairwise$padj<sigpadj & abs(respairwise$log2FoldChange)>=siglfc,])[1:50]	# Select the first 30 significant hits (sorted by padj)
sig <- sig[!is.na(sig)]					
