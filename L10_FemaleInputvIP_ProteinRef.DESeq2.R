#!/usr/bin/env Rscript
# variables
cond1 <- "L10FemaleInput"
cond2 <- "L10FemaleIP"

output <- "DESeq2Scripts/L10_Female_IPvInputall_Proteinref_"
...
samples <- read.table("Kallistosheets_cp/L10_Female_IPvInput.txt", header=TRUE)
samples
files <- file.path(dir, samples$sample, "abundance.h5")
files
names(files) <- paste0(samples$name, 1:19)
tx2gene <- read.table("/home/groups/nirao/Bioinformatics3/Bioinformatics/vM21_Tx2Gene_Final.txt")
head(tx2gene)
txi.kallisto <- tximport(files, type="kallisto", tx2gene=tx2gene)

sampleTable <- data.frame(condition = factor(c(rep(cond1, 9), rep(cond2, 10))))
