...
cond1 <- "Primed"
cond2 <- "Unprimed"
output <- "DESeq2Scripts/Kallisto_L10proteinref_VMH_Primedvunprimed_4v3_fullref_local"
...
dir <- "/scratch/users/knoedler/All_Copied_Kallisto/"
samples <- read.table("NewKallistoSheets/L10VMH_PrimedvUnprimed.txt", header=TRUE)
samples
files <- file.path(dir, samples$sample, "abundance.h5")
files
names(files) <- paste0(samples$name, 1:7)
tx2gene <- read.table("/home/groups/nirao/Bioinformatics3/Bioinformatics/vM17.tx2gene.FINAL.txt")
head(tx2gene)
