#!/usr/bin/env Rscript
# variables
cond1 <- "L10FemaleInputPrimed"
cond2 <- "L10FemaleInputUnprimed"

output <- "DESeq2Scripts/L10_Female_InputPrimedvUnprimed_Proteinref_flipped"
...
samples <- read.table("Kallistosheets_cp/L10_Female_Input_PrimedvUnprimed_flipped.txt", header=TRUE)
samples
...
sampleTable <- data.frame(condition = factor(c(rep(cond1, 5), rep(cond2, 4))))
