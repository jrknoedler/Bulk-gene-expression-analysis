...
cond1 <- "BNSTPrimed"
cond2 <- "MaleBNST"
output <- "All_RNASeq_Analyses/DESeq2/Kallisto_Esr1Adult_BNST_PrimedvMale_3v3_localvsd"
...
samples <- read.table("NewKallistoSheets/AdultEsr1Kallisto_BNST_FemalePrimedvMale_3v3S.tsv", header=TRUE)
samples
...
sampleTable <- data.frame(condition = factor(c(rep(cond1, 3), rep(cond2, 3))), batch=factor(c(rep("1"),rep("1"),rep("2"),rep("2"),rep("1"),rep("2"))))
