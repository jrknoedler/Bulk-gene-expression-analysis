...
output <- "All_RNASeq_Analyses/DESeq2/AllAdultEsr1_FULL_vsd_deopt"
...
dir <- "/scratch/users/knoedler/AdultEsr1Deopt_ALL/"
samples <- read.table("NewKallistoSheets/AdultEsr1Kallisto_FULL.tsv", header=TRUE)
samples
files <- file.path(dir, samples$sample, "abundance.h5")
files
names(files) <- paste0(samples$name, 1:38)
tx2gene <- read.table("Bioinformatics2/Bioinformatics/vM17.tx2gene.FINAL.txt")
head(tx2gene)
txi.kallisto <- tximport(files, type="kallisto", tx2gene=tx2gene)

sampleTable <- data.frame(condition = factor(c(rep(cond1, 3), rep(cond2, 3), rep(cond3, 4), rep(cond4, 3), rep(cond5, 3), rep(cond6, 4), rep(cond7, 3), rep(cond8, 3), rep(cond9, 3), rep(cond10, 3), rep(cond11, 3), rep(cond12, 3))))
