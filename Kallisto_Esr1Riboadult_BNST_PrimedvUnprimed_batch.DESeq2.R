...
cond1 <- "BNSTPrimed"
cond2 <- "BNSTUnprimed"
output <- "All_RNASeq_Analyses/DESeq2/Kallisto_Esr1Adult_BNST_PrimedvUnprimed_deopt_batch"
...
samples <- read.table("NewKallistoSheets/AdultEsr1Kallisto_BNSTPrimedvUnprimedFULL.tsv", header=TRUE)
samples
...
sampleTable <- data.frame(condition = factor(c(rep(cond1, 3), rep(cond2, 3))), batch = factor(c(rep("1"), rep("1"), rep("2"), rep("1"), rep("1"), rep("2"))))
