...
cond4 <- "UnprimedIP"
cond5 <- "MaleIP"
output <- "All_RNASeq_Analyses/DESeq2/Kallisto_L10_all_deopt_tile_21"
...
dir <- "/scratch/users/knoedler/L10_Kallisto_vm17_Deopt_Tiles/"
samples <- read.table("NewKallistoSheets/AllL10VMHAdult_21.txt", header=TRUE)
samples
files <- file.path(dir, samples$sample, "abundance.h5")
files
names(files) <- paste0(samples$name, 1:21)
tx2gene <- read.table("Bioinformatics2/Bioinformatics/vM17.tx2gene.FINAL.txt")
head(tx2gene)
txi.kallisto <- tximport(files, type="kallisto", tx2gene=tx2gene)

sampleTable <- data.frame(condition = factor(c(rep(cond1, 4), rep(cond2, 4), rep(cond3, 5), rep(cond4, 5), rep(cond5, 3))))
...
respairwise = results(dds, contrast=c("condition",cond1,cond5))																		# Select comparison to perform (for log2 changes)
