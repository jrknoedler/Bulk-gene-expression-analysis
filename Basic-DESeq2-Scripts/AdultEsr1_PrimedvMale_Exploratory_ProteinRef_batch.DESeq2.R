...
cond7 <- "MalePOA"
cond8 <- "MaleVMH"
output <- "DESeq2Scripts/Esr1Adult_PrimedvMale_GLM_batch_dissection"
...
sampleTable <- data.frame(condition = factor(c(rep(cond1, 3), rep(cond2, 3), rep(cond3, 3), rep(cond4, 3), rep(cond5, 3), rep(cond6, 3), rep(cond7, 3), rep(cond8, 3))), dissection= factor(c(rep("1"),rep("2"),rep("3"),rep("1"),rep("2"),rep("3"),rep("1"),rep("2"),rep("3"),rep("1"),rep("2"),rep("3"),rep("7"),rep("5"),rep("6"),rep("4"),rep("5"),rep("6"),rep("4"),rep("5"),rep("6"),rep("4"),rep("5"),rep("6"))))
rownames(sampleTable) <- colnames(txi.kallisto$counts)


# Run DESeq2 to get the result for differential-expression
dds=DESeqDataSetFromTximport(txi.kallisto, sampleTable, design =~dissection+condition)
...
resBNST <- results(dds, contrast=c("condition",cond1,cond5))
...
resMeA <- results(dds, contrast=c("condition",cond2,cond6))
...
resPOA <- results(dds, contrast=c("condition",cond3,cond7))
