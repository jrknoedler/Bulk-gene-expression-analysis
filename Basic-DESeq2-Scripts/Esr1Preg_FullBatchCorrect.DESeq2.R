...
output <- "DESeq2Scripts/Esr1Adult_PregGLM_sex_"
...
samples <- read.table("Kallistosheets/AllAdultEsr1preg_nooutlier.tsv", header=TRUE)
samples
files <- file.path(dir, samples$sample, "abundance.h5")
files
names(files) <- paste0(samples$name, 1:46)
tx2gene <- read.table("/home/groups/nirao/Bioinformatics3/Bioinformatics/vM21_Tx2Gene_Updated.txt")
head(tx2gene)
txi.kallisto <- tximport(files, type="kallisto", tx2gene=tx2gene)

sampleTable <- data.frame(condition = factor(c(rep(cond1, 3), rep(cond2, 3), rep(cond3, 3), rep(cond4, 3),rep(cond5, 3), rep(cond6, 3), rep(cond7, 3), rep(cond8, 3), rep(cond9, 3), rep(cond10, 3), rep(cond11, 3), rep(cond12, 3), rep(cond13, 3), rep(cond14, 2), rep(cond15, 2), rep(cond16, 3))), region=factor(c(rep("POA",9), rep("BNST",9), rep("MeA", 9), rep("VMH",9), rep("BNST",3),rep("MeA",2),rep("POA",2), rep("VMH",3))), batch = factor(c(rep("1"),rep("1"), ("2"),rep("1"),rep("1"), ("2"),rep("1"),rep("1"), ("2"),rep("1"),rep("1"), ("2"),rep("1"),rep("1"), ("2"),rep("1"),rep("1"), ("2"),rep("1"),rep("1"), ("2"),rep("1"),rep("1"), ("2"),rep("1"),rep("1"), ("2"),rep("1"),rep("1"), ("2"),rep("1"),rep("1"), ("2"),rep("1"),rep("1"), ("2"), rep("3",10))), sex=factor(c(rep("Male",3), rep("Female",6), rep("Male",3), rep("Female",6), rep("Male",3), rep("Female",6), rep("Male", 3), rep("Female",16))), state=factor(c(rep("Intact",3), rep("Primed",3), rep("Unprimed",3), rep("Intact",3), rep("Primed",3), rep("Unprimed",3), rep("Intact",3), rep("Primed",3), rep("Unprimed",3), rep("Intact",3), rep("Primed",3), rep("Unprimed",3),rep("Pregnant",10)))) 
...
dds=DESeqDataSetFromTximport(txi.kallisto, sampleTable, design =~0 + sex:region + state:region)
