...
cond5 <- "GroupMaleIP"
cond6 <- "SingleMaleIP"
output <- "All_RNASeq_Analyses/DESeq2/Kallisto_L10_all_deopt_tile_25"
...
samples <- read.table("NewKallistoSheets/AllL10VMHAdult_25.txt", header=TRUE)
samples
files <- file.path(dir, samples$sample, "abundance.h5")
files
names(files) <- paste0(samples$name, 1:25)
tx2gene <- read.table("Bioinformatics2/Bioinformatics/vM17.tx2gene.FINAL.txt")
head(tx2gene)
txi.kallisto <- tximport(files, type="kallisto", tx2gene=tx2gene)

sampleTable <- data.frame(condition = factor(c(rep(cond1, 4), rep(cond2, 4), rep(cond3, 5), rep(cond4, 5), rep(cond5, 3), rep(cond6, 4))))
