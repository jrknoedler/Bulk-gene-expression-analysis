...
cond15 <- "CastratePOA"
cond16 <- "CastrateVMH"
output <- "DESeq2Scripts/Esr1Adult_FULL_GLM_withcast_original_region_extended"
...
samples <- read.table("Kallistosheets_cp/AdultEsr1Kallisto_FULL.tsv", header=TRUE)
samples
files <- file.path(dir, samples$sample, "abundance.h5")
files
names(files) <- paste0(samples$name, 1:48)
tx2gene <- read.table("/home/groups/nirao/Bioinformatics3/Bioinformatics/vM21_Tx2Gene_Final.txt")
head(tx2gene)
txi.kallisto <- tximport(files, type="kallisto", tx2gene=tx2gene)

sampleTable <- data.frame(group=factor(c(rep("Primed", 3), rep("Unprimed", 3), rep("IntactMale",12), rep("Primed", 9), rep("Unprimed", 9), rep("CastrateMale", 12))), condition = factor(c(rep(cond1, 3), rep(cond2, 3), rep(cond3, 3), rep(cond4, 3), rep(cond5, 3), rep(cond6, 3), rep(cond7, 3), rep(cond8, 3), rep(cond9, 3), rep(cond10, 3), rep(cond11, 3), rep(cond12, 3), rep(cond13, 3), rep(cond14, 3), rep(cond15, 3), rep(cond16, 3))), batch= factor(c(rep("1"), rep("1"),rep("2"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("2"), rep("1"),rep("1"),rep("4"),rep("1"),rep("1"),rep("4"),rep("1"),rep("1"),rep("4"),rep("1"),rep("1"),rep("4"))), region= factor(c(rep("BNST", 9), rep("MeA",3), rep("POA",3), rep("VMH",3), rep("MeA",3), rep("POA",3), rep("VMH",3), rep("MeA",3), rep("POA",3), rep("VMH",3), rep("BNST", 3), rep("MeA",3), rep("POA",3), rep("VMH",3))), dissection.nested = factor(c(rep("1"), rep("2"), rep("3"), rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"), rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"),rep("1"), rep("2"), rep("3"))))
rownames(sampleTable) <- colnames(txi.kallisto$counts)

# Run DESeq2 to get the result for differential-expression
dds=DESeqDataSetFromTximport(txi.kallisto, sampleTable, design=~0+region:group+group:dissection.nested)
...
resBNST1 <- results(dds, contrast=list("groupPrimed.regionBNST","groupIntactMale.regionBNST"))
...
resBNST2 <- results(dds, contrast=list("groupIntactMale.regionBNST","groupUnprimed.regionBNST"))
...
resBNST3 <- results(dds, contrast=list("groupPrimed.regionBNST","groupUnprimed.regionBNST"))
...
resMeA1 <- results(dds, contrast=list("groupPrimed.regionMeA","groupIntactMale.regionMeA"))
...
resMeA2 <- results(dds, contrast=list("groupUnprimed.regionMeA","groupIntactMale.regionMeA"))
table(resMeA2$padj<sigpadj)
## Order by adjusted p-value
resMeA2 <- resMeA2[order(resMeA1$padj), ]
## Merge with normalized count data
...
resMeA3 <- results(dds, contrast=list("groupPrimed.regionMeA","groupUnprimed.regionMeA"))
table(resMeA3$padj<sigpadj)
## Order by adjusted p-value
resMeA3 <- resMeA3[order(resMeA3$padj), ]
## Merge with normalized count data
resMeA1data <- merge(as.data.frame(resMeA3), as.data.frame(counts(dds, normalized=TRUE)), by="row.names", sort=FALSE)
names(resMeA3data)[1] <- "Gene"
## Write results
write.csv(resMeA1data, file=paste0(output, "MeA_PvU-ExpDiff.csv"))
resPOA1 <- results(dds, contrast=list("groupPrimed.regionPOA","groupIntactMale.regionPOA"))
...
resPOA2 <- results(dds, contrast=list("groupUnprimed.regionPOA","groupIntactMale.regionPOA"))
...
resPOA3 <- results(dds, contrast=list("groupPrimed.regionPOA","groupUnprimed.regionPOA"))
...
resVMH1 <- results(dds, contrast=list("groupPrimed.regionVMH","groupIntactMale.regionVMH"))
...
resVMH2 <- results(dds, contrast=list("groupUnprimed.regionVMH","groupIntactMale.regionVMH"))
...
resVMH3 <- results(dds, contrast=list("groupPrimed.regionVMH","groupUnprimed.regionVMH"))
...
resBNST4 <- results(dds, contrast=list("groupIntactMale.regionBNST","groupCastrateMale.regionBNST"))
table(resBNST4$padj<sigpadj)
## Order by adjusted p-value
resBNST4 <- resBNST4[order(resBNST4$padj), ]
## Merge with normalized count data
resBNST4data <- merge(as.data.frame(resBNST4), as.data.frame(counts(dds, normalized=TRUE)), by="row.names", sort=FALSE)
names(resBNST4data)[1] <- "Gene"
## Write results
write.csv(resBNST4data, file=paste0(output, "BNST_MvC-ExpDiff.csv"))
resMeA4 <- results(dds, contrast=list("groupIntactMale.regionMeA","groupCastrateMale.regionMeA"))
table(resMeA4$padj<sigpadj)
## Order by adjusted p-value
resMeA4 <- resMeA4[order(resMeA4$padj), ]
## Merge with normalized count data
resMeA4data <- merge(as.data.frame(resMeA4), as.data.frame(counts(dds, normalized=TRUE)), by="row.names", sort=FALSE)
names(resMeA4data)[1] <- "Gene"
## Write results
write.csv(resMeA4data, file=paste0(output, "MeA_MvC-ExpDiff.csv"))
resPOA4 <- results(dds, contrast=list("groupIntactMale.regionPOA","groupCastrateMale.regionPOA"))
table(resPOA4$padj<sigpadj)
## Order by adjusted p-value
resPOA4 <- resPOA4[order(resPOA4$padj), ]
## Merge with normalized count data
resPOA4data <- merge(as.data.frame(resPOA4), as.data.frame(counts(dds, normalized=TRUE)), by="row.names", sort=FALSE)
names(resPOA4data)[1] <- "Gene"
## Write results
write.csv(resPOA4data, file=paste0(output, "POA_MvC-ExpDiff.csv"))
resVMH4 <- results(dds, contrast=list("groupIntactMale.regionVMH","groupCastrateMale.regionVMH"))
table(resVMH4$padj<sigpadj)
## Order by adjusted p-value
resVMH4 <- resVMH4[order(resVMH4$padj), ]
## Merge with normalized count data
resVMH4data <- merge(as.data.frame(resVMH4), as.data.frame(counts(dds, normalized=TRUE)), by="row.names", sort=FALSE)
names(resVMH4data)[1] <- "Gene"
## Write results
write.csv(resVMH4data, file=paste0(output, "VMH_MvC-ExpDiff.csv"))
