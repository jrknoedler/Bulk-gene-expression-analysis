...
cond15 <- "POApreg"
cond16 <- "VMHpreg"
output <- "DESeq2Scripts/All_Esr1adultwpreg_forPCA_Labmtg"
...
samples <- read.table("Kallistosheets/AllAdultEsr1preg.tsv", header=TRUE)
samples
...
sampleTable <- data.frame(condition = factor(c(rep(cond1, 3), rep(cond2, 3), rep(cond3, 3), rep(cond4, 3), rep(cond5, 3), rep(cond6, 3), rep(cond7, 3), rep(cond8, 3), rep(cond9, 3), rep(cond10, 3), rep(cond11, 3), rep(cond12, 3), rep(cond13, 3), rep(cond14, 2), rep(cond15, 2), rep(cond16, 3))))
...
garbage <- dev.off() # Save to file
pdf(paste0(output, "all-PCA_top2000.pdf"))
...
pheatmap(as.matrix(sampleDists), key=FALSE, symm=TRUE, trace="none", col=rev(dmcol), margin=c(15, 15), main="all samples Matrix")
garbage <- dev.off() # Save to file
