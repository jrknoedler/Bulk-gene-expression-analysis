...
cond1 <- "P15POAIP"
cond2 <- "P15POAInput"
output <- "All_RNASeq_Analyses/DESeq2/Kallisto_P15_POAInputvIP_sex"
...
samples <- read.table("NewKallistoSheets/P15POA_Full.txt", header=TRUE)
samples
...
sampleTable <- data.frame(condition = factor(c(rep(cond1, 4), rep(cond2, 4))), sex = factor(c(rep("Female", 2), rep("Male",2), rep("Female",2), rep("Male",2))))
rownames(sampleTable) <- colnames(txi.kallisto$counts)
sampleTable

# Run DESeq2 to get the result for differential-expression
dds=DESeqDataSetFromTximport(txi.kallisto, sampleTable, design =~sex+condition)
