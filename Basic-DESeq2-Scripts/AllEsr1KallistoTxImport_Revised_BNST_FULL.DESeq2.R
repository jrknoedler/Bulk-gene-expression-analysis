...
cond2 <- "BNSTUnprimed"
cond3 <- "MaleBNST"
cond4 <- "P15FemaleBNSTIP"
cond5 <- "P15MaleBNSTIP"
cond6 <- "P15FemaleBNSTInput"
cond7 <- "P15MaleBNSTInput"

output <- "All_RNASeq_Analyses/DESeq2/AllEsr1_FULL_BNSTall_NewDeopt_rlog_top50"
...
samples <- read.table("NewKallistoSheets/Compiled_Esr1_BNST_Kallisto.txt", header=TRUE)
samples
...
sampleTable <- data.frame(condition = factor(c(rep(cond1, 3), rep(cond2, 3), rep(cond3, 2), rep(cond4, 2), rep(cond5, 2), rep(cond6, 2), rep(cond7, 2))))
