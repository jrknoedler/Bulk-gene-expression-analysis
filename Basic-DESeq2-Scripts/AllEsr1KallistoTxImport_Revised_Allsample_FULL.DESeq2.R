...
cond11 <- "VMHPrimed"
cond12 <- "VMHUnprimed"
cond13 <- "P15FemaleBNSTIP"
cond14 <- "P15MaleBNSTIP"
cond15 <- "P15FemaleMeAIP"
cond16 <- "P15MaleMeAIP"
cond17 <- "P15FemalePOAIP"
cond18 <- "P15MalePOAIP"
cond19 <- "P15FemaleVMHIP"
cond20 <- "P15MaleVMHIP"
cond21 <- "P15FemaleBNSTInput"
cond22 <- "P15MaleBNSTInput"
cond23 <- "P15FemaleMeAInput"
cond24 <- "P15MaleMeAInput"
cond25 <- "P15FemalePOAInput"
cond26 <- "P15MalePOAInput"
cond27 <- "P15FemaleVMHInput"
cond28 <- "P15MaleVMHInput"

output <- "All_RNASeq_Analyses/DESeq2/AllEsr1_FULL_NewDeopt_"
...
dir <- "/scratch/users/knoedler/All_Esr1_Deopt/"
samples <- read.table("NewKallistoSheets/Compiled_Esr1_Kallisto.txt", header=TRUE)
samples
files <- file.path(dir, samples$sample, "abundance.h5")
files
names(files) <- paste0(samples$name, 1:66)
tx2gene <- read.table("Bioinformatics3/Bioinformatics/vM17.tx2gene.FINAL.txt")
head(tx2gene)
txi.kallisto <- tximport(files, type="kallisto", tx2gene=tx2gene)

sampleTable <- data.frame(condition = factor(c(rep(cond1, 3), rep(cond2, 3), rep(cond3, 2), rep(cond4, 3), rep(cond5, 3), rep(cond6, 3), rep(cond7, 3), rep(cond8, 3), rep(cond9, 3), rep(cond10, 3), rep(cond11, 3), rep(cond12, 2), rep(cond13, 2), rep(cond14, 2), rep(cond15, 2), rep(cond16, 2), rep(cond17, 2), rep(cond18, 2), rep(cond19, 2), rep(cond20, 2), rep(cond21, 2), rep(cond22, 2), rep(cond23, 2), rep(cond24, 2), rep(cond25, 2), rep(cond26, 2), rep(cond27, 2), rep(cond28, 2))))
...
plotPCA(rld, intgroup="condition",ntop=200)
...
sig <- rownames(respairwise[!is.na(respairwise$padj) & respairwise$padj<sigpadj & abs(respairwise$log2FoldChange)>=siglfc,])[1:200]	# Select the first 30 significant hits (sorted by padj)
sig <- sig[!is.na(sig)]					
