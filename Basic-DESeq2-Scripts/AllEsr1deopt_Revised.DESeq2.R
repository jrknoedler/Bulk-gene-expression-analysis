...
output <- "All_RNASeq_Analyses/DESeq2/AllAdultRiboIP_deopt_"
...
dir <- "/scratch/users/knoedler/AdultRibo_vm17_all_Deopt/"
samples <- read.table("NewKallistoSheets/AdultEsr1Kallisto_deopt.tsv", header=TRUE)
samples
...
sampleTable <- data.frame(condition = factor(c(rep(cond1, 2), rep(cond2, 2), rep(cond3, 2), rep(cond4, 2), rep(cond5, 2), rep(cond6, 2), rep(cond7, 2), rep(cond8, 2), rep(cond9, 2), rep(cond10, 2), rep(cond11, 2), rep(cond12, 2))))
