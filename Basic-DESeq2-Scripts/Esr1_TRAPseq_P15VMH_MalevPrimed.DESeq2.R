...
output <- "DESeq2Scripts/Esr1_TRAP_P15VMH_InputvIP"
...
samples <- read.table("NewKallistoSheets/P15VMH_InputvIP.txt", header=TRUE)
samples
files <- file.path(dir, samples$sample, "abundance.h5")
files
names(files) <- paste0(samples$name, 1:8)
tx2gene <- read.table("/home/groups/nirao/Bioinformatics3/Bioinformatics/vM21_Tx2Gene_Final.txt")
head(tx2gene)
txi.kallisto <- tximport(files, type="kallisto", tx2gene=tx2gene)

sampleTable <- data.frame(condition = factor(c(rep(cond1, 4), rep(cond2, 4))))
...
with(resdata, plot(log2FoldChange, -log10(pvalue), pch=20, main=main))
with(subset(resdata, padj<sigthresh ), points(log2FoldChange, -log10(pvalue), pch=20, col="red"))
with(subset(resdata, abs(log2FoldChange)>lfcthresh), points(log2FoldChange, -log10(pvalue), pch=20, col="orange"))
  with(subset(resdata, padj<sigthresh & abs(log2FoldChange)>lfcthresh), points(log2FoldChange, -log10(pvalue), pch=20, col="green"))
  if (labelsig) {
    require(calibrate)
    with(subset(resdata, padj<sigthresh & abs(log2FoldChange)>lfcthresh), textxy(log2FoldChange, -log10(pvalue), labs=Gene, cex=textcx))
  }
  legend(legendpos, xjust=1, yjust=1, legend=c(paste("FDR<",sigthresh,sep=""), paste("|LogFC|>",lfcthresh,sep=""), "both"), pch=20, col=c("red","orange","green"))
}
png(paste0(output, "-diffexpr-volcanoplot.png", 1200, 1000, pointsize=20))
