...
cond1 <- "IntactMale"
cond2 <- "PrimedFemale"
cond3 <- "UnprimedFemale"
output <- "DESeq2Scripts/Esr1Adult_BNST_GLM_batch_adjust"
...
samples <- read.table("Kallistosheets_cp/AdultEsr1Kallisto_BNST_3way.tsv", header=TRUE)
samples
...
sampleTable <- data.frame(condition = factor(c(rep(cond1, 3), rep(cond2, 3), rep(cond3, 3))), batch= factor(c(rep("1"), rep("1"),rep("2"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("2"))))
...
resMvP <- results(dds, contrast=c("condition",cond1,cond2))
table(resMvP$padj<sigpadj)
## Order by adjusted p-value
resMvP <- resMvP[order(resMvP$padj), ]
## Merge with normalized count data
resMvPdata <- merge(as.data.frame(resMvP), as.data.frame(counts(dds, normalized=TRUE)), by="row.names", sort=FALSE)
names(resMvPdata)[1] <- "Gene"
## Write results
write.csv(resMvPdata, file=paste0(output, "MalevPrimed-ExpDiff.csv"))
resMvU <- results(dds, contrast=c("condition",cond1,cond3))
table(resMvU$padj<sigpadj)
## Order by adjusted p-value
resMvU <- resMvU[order(resMvU$padj), ]
## Merge with normalized count data
resMvUdata <- merge(as.data.frame(resMvU), as.data.frame(counts(dds, normalized=TRUE)), by="row.names", sort=FALSE)
names(resMvUdata)[1] <- "Gene"
## Write results
write.csv(resMvUdata, file=paste0(output, "MalevUnprimed-ExpDiff.csv"))
resPvU <- results(dds, contrast=c("condition",cond2,cond3))
table(resPvU$padj<sigpadj)
## Order by adjusted p-value
resPvU <- resPvU[order(resPvU$padj), ]
## Merge with normalized count data
resPvUdata <- merge(as.data.frame(resPvU), as.data.frame(counts(dds, normalized=TRUE)), by="row.names", sort=FALSE)
names(resPvUdata)[1] <- "Gene"
## Write results
write.csv(resPvUdata, file=paste0(output, "PrimedvUnprimed-ExpDiff.csv"))
#resVMH <- results(dds, contrast=c("condition",cond4,cond8))
