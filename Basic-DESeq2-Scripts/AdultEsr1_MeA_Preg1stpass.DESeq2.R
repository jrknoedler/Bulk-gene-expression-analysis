...
cond2 <- "Estrus"
cond3 <- "Diestrus"
output <- "DESeq2Scripts/Esr1Adult_MeA_PregvEst_3way_dropY_droplow_dropmtnooutlier"
...
samples <- read.table("Kallistosheets/MeA_Preg.tsv", header=TRUE)
samples
...
sampleTable <- data.frame(condition = factor(c(rep(cond1, 2), rep(cond2, 3), rep(cond3, 3))))
...
## Write results
write.csv(resMalevPrimeddata, file=paste0(output, "_PregvEstrus-ExpDiff.csv"))
...
## Write results
write.csv(resPrimedvUnprimeddata, file=paste0(output, "Estrusvdiestrus-ExpDiff.csv"))
...
## Write results
write.csv(resMalevUnprimeddata, file=paste0(output, "Pregvdiestrus-ExpDiff.csv"))
