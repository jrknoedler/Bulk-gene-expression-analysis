...
# variables
table <- "AdultRiboEsr1NoDedupOut2/DESeq2/BNSTUnprimed_vs_MaleBNST.genes.matrix"
cond1 <- "Primed"
rep1 <- 2
cond2 <- "Unprimed"
rep2 <- 2
output <- "All_RNASeq_Analyses/DESeq2/Kallisto_L10_PrimedVUnprimed_Deopt_tile_adjacent_batch"
...
samples <- read.table("NewKallistoSheets/AllL10VMHAdult_FemaleIP.txt", header=TRUE)
samples
files <- file.path(dir, samples$sample, "abundance.h5")
files
names(files) <- paste0(samples$name, 1:7)
tx2gene <- read.table("Bioinformatics2/Bioinformatics/vM17.tx2gene.FINAL.txt")
head(tx2gene)
txi.kallisto <- tximport(files, type="kallisto", tx2gene=tx2gene)

sampleTable <- data.frame(condition = factor(c(rep(cond1, 4), rep(cond2, 3))), batch=factor(c(rep("1"),rep("1"),rep("1"),rep("2"),rep("1"),rep("1"),rep("2"))))
rownames(sampleTable) <- colnames(txi.kallisto$counts)


# Run DESeq2 to get the result for differential-expression
dds=DESeqDataSetFromTximport(txi.kallisto, sampleTable, ~batch+condition)
...
condcol <- brewer.pal(8, "Set1")
dmcol <- colorRampPalette(brewer.pal(9, 'GnBu'))(100)
sampleDists <- as.matrix(dist(t(assay(rld))))

pdf(paste0(output, cond1, "_vs_", cond2, "-DistanceMatrix.pdf"))
heatmap.2(as.matrix(sampleDists), key=FALSE, symm=TRUE, trace="none", col=rev(dmcol), margin=c(15, 15), main=paste0(cond1, " vs ", cond2, " Matrix"))
garbage <- dev.off() # Save to file
