...
output <- "DESeq2Scripts/Esr1Adult_POA_GLM_4way_nooutlier"
...
samples <- read.table("Kallistosheets_cp/AdultEsr1Kallisto_POAFULLcas_reduced.tsv", header=TRUE)
samples
files <- file.path(dir, samples$sample, "abundance.h5")
files
names(files) <- paste0(samples$name, 1:10)
tx2gene <- read.table("/home/groups/nirao/Bioinformatics3/Bioinformatics/vM21_Tx2Gene_Final.txt")
head(tx2gene)
txi.kallisto <- tximport(files, type="kallisto", tx2gene=tx2gene)

sampleTable <- data.frame(condition = factor(c(rep(cond1, 3), rep(cond2, 2), rep(cond3, 3), rep(cond4, 2))))
