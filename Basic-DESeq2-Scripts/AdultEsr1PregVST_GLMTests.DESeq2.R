...
output <- "DESeq2Scripts/Esr1Adult_All_UpdatePreg_WCGNA_string"
...
samples <- read.table("Kallistosheets/AllAdultEsr1preg_WCGNA.tsv", header=TRUE)
samples
...
sampleTable <- data.frame(condition = factor(c(rep(cond1, 3), rep(cond2, 3), rep(cond3, 3), rep(cond5, 3), rep(cond6, 3), rep(cond7, 3), rep(cond8, 3), rep(cond10, 3), rep(cond11, 3), rep(cond12, 3), rep(cond13, 3), rep(cond15, 3), rep(cond16, 3), rep(cond17,3), rep(cond18, 3), rep(cond20,3))))
...
write.csv(assay(rld), file=paste0(output,"_Variance_Stabilized_Counts.csv"))
# Save differential expression results
resMalevPrimed <- results(dds, contrast=c("region:gravid","regionBNST.gravidyes","regionBNSTgravidno"))
...
resMalevCastrate <- results(dds, contrast=c("condition",cond1,cond4))
table(resMalevCastrate$padj<sigpadj)
## Order by adjusted p-value
resMalevCastrate <- resMalevCastrate[order(resMalevCastrate$padj), ]
## Merge with normalized count data
resMalevCastratedata <- merge(as.data.frame(resMalevCastrate), as.data.frame(counts(dds, normalized=TRUE)), by="row.names", sort=FALSE)
names(resMalevCastrate)[1] <- "Gene"
## Write resuls
write.csv(resMalevCastratedata, file=paste0(output, "MvC-ExpDiff.csv"))
resPrimedvCastrate <- results(dds, contrast=c("condition",cond2,cond4))
table(resPrimedvCastrate$padj<sigpadj)
## Order by adjusted p-value
resPrimedvCastrate <- resPrimedvCastrate[order(resPrimedvCastrate$padj), ]
## Merge with normalized count data
resPrimedvCastratedata <- merge(as.data.frame(resPrimedvCastrate), as.data.frame(counts(dds, normalized=TRUE)), by="row.names", sort=FALSE)
names(resPrimedvCastrate)[1] <- "Gene"
## Write resuls
write.csv(resPrimedvCastratedata, file=paste0(output, "PvC-ExpDiff.csv"))

resUnprimedvCastrate <- results(dds, contrast=c("condition",cond3,cond4))
table(resUnprimedvCastrate$padj<sigpadj)
## Order by adjusted p-value
resUnprimedvCastrate <- resUnprimedvCastrate[order(resUnprimedvCastrate$padj), ]
## Merge with normalized count data
resUnprimedvCastratedata <- merge(as.data.frame(resUnprimedvCastrate), as.data.frame(counts(dds, normalized=TRUE)), by="row.names", sort=FALSE)
names(resPrimedvCastrate)[1] <- "Gene"
## Write resuls
write.csv(resUnprimedvCastratedata, file=paste0(output, "UvC-ExpDiff.csv"))
