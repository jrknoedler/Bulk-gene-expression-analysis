...
cond1 <- "VMHPrimed"
cond2 <- "VMHUnprimed"
output <- "All_RNASeq_Analyses/DESeq2/Kallisto_Esr1Adult_VMH_PrimedvUnprimed_deopt_fixed"
...
samples <- read.table("NewKallistoSheets/AdultEsr1Kallisto_VMH_PrimedvUnprimed_fixed.tsv", header=TRUE)
samples
files <- file.path(dir, samples$sample, "abundance.h5")
files
names(files) <- paste0(samples$name, 1:5)
tx2gene <- read.table("Bioinformatics2/Bioinformatics/vM17.tx2gene.FINAL.txt")
head(tx2gene)
txi.kallisto <- tximport(files, type="kallisto", tx2gene=tx2gene)

sampleTable <- data.frame(condition = factor(c(rep(cond1, 3), rep(cond2, 2))))
