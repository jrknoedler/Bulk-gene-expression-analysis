...
output <- "DESeq2Scripts/All_L10_IP_vsd"
siglfc <- 0.5		# |Log2| fold change to be used as significant. Set at 1 by default, meaning a 2 fold change (log2(2)=1)
...
samples <- read.table("Kallistosheets_cp/L10_AllMaleFemaleIP.txt", header=TRUE)
samples
files <- file.path(dir, samples$sample, "abundance.h5")
files
names(files) <- paste0(samples$name, 1:13)
tx2gene <- read.table("/home/groups/nirao/Bioinformatics3/Bioinformatics/vM21_Tx2Gene_Final.txt")
head(tx2gene)
txi.kallisto <- tximport(files, type="kallisto", tx2gene=tx2gene)

sampleTable <- data.frame(condition = factor(c(rep(cond1, 4), rep(cond2, 3), rep(cond3, 3), rep(cond4, 3))))
