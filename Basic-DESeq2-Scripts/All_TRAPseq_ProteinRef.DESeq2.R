...
cond19 <- "P15FemaleVMH"
cond20 <- "P15MaleVMH"
cond21 <- "L10PrimedFemale"
cond22 <- "L10UnprimedFemale"
cond23 <- "L10GroupMale"
cond24 <- "L10SingleMale"

output <- "DESeq2Scripts/All_TRAP_ProteinReftest_compmod"
...
samples <- read.table("NewKallistoSheets/All_IP_Kallisto.tsv", header=TRUE)
samples
files <- file.path(dir, samples$sample, "abundance.h5")
files
names(files) <- paste0(samples$name, 1:69)
tx2gene <- read.table("/home/groups/nirao/Bioinformatics3/Bioinformatics/vM21_Tx2Gene_Final.txt")
head(tx2gene)
txi.kallisto <- tximport(files, type="kallisto", tx2gene=tx2gene)

sampleTable <- data.frame(condition = factor(c(rep(cond1, 3), rep(cond2, 3), rep(cond3, 4), rep(cond4, 4), rep(cond5, 3), rep(cond6, 3), rep(cond7, 3), rep(cond8, 3), rep(cond9, 3), rep(cond10, 3), rep(cond11, 3), rep(cond12, 2), rep(cond13, 2),rep(cond14, 2),rep(cond15, 2),rep(cond16, 2),rep(cond17, 2),rep(cond18, 2),rep(cond19, 2),rep(cond20, 2),rep(cond21, 5),rep(cond22, 5),rep(cond23, 3),rep(cond24, 4))))
