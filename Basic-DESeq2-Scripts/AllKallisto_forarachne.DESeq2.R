...
output <- "DESeq2Scripts/AllKallisto_forArachne"
...
samples <- read.table("Kallistosheets/AllKallisto.tsv", header=TRUE)
samples
files <- file.path(dir, samples$Sample, "abundance.h5")
files
names(files) <- paste0(samples$name, 1:118)
...
sampleTable <- data.frame(condition = factor(c(rep(cond1, 59), rep(cond2, 59))))
...
dds <- DESeq(dds, minReplicatesForReplace = 60)
# run DESeq2
resultsNames(dds)
...
write.csv(assay(vst), file=paste0(output,"_vst_counts.csv"))
