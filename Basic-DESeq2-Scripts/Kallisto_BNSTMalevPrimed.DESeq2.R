...
cond2 <- "MaleBNST"
rep2 <- 2
output <- "AdultRiboEsr1NoDedupOut2/DESeq2/Kallisto_BNSTMaleVPrimed_"
...
dir <- "/scratch/users/knoedler/AdultRiboKallisto_vM17/"
samples <- read.table("AdultKallistoDataSheets/AdultKallistoBNST_MalevPrimed.tsv", header=TRUE)
samples
files <- file.path(dir, samples$sample, "abundance.h5")
files
names(files) <- paste0(samples$name, 1:4)
tx2gene <- read.table("Bioinformatics/vM17.tx2gene.FINAL.txt")
head(tx2gene)
